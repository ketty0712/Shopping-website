    ...
    <script>
        ...
        $(document).ready(function($) {
            ...
            $(".input-group").find("input").keyup(function() {
                ...
                //改小計
                var new_tot = count * parseInt(price[ind]);
            ...
            $("[name='add']").click(function() {
                ...
                max = parseInt($(this).closest(".input-group").find('[name="p_q"]').html());
                count = parseInt(this_item_cnt.val()); //單個商品數量框的值
                if (count <= max) {
                    count++; //數量增加
                    this_item_cnt.attr({
                        "value": count
            ...
            if($(window).width()<768){
                $('.table-header').find('div').eq(2).removeClass('col-sm-2');
                $('.table-header').find('div').eq(2).addClass('col-sm-3');
                $('.item-quantity').removeClass('col-sm-2');
                $('.item-quantity').addClass('col-sm-3');
            }
            $(window).resize(function(){
                if($(window).width()<768){
                    ...
                    $('.table-header').find('div').eq(2).removeClass('col-sm-3');
                    $('.table-header').find('div').eq(2).addClass('col-sm-2');
                    $('.item-quantity').removeClass('col-sm-3');
                    $('.item-quantity').addClass('col-sm-2');
                }
            });
        }); //這個是document的括號
                    ...
                    if (isset($_SESSION['cart_id']))
                        for ($i = 0; $i < count($_SESSION['cart_id']); $i++) {
                            ...
                            <div class="table-row row cart-item">
                                <div class="col-xs-12 col-sm-5 text-center" style="vertical-align: middle; margin:auto 0; display:flex;align-items:center;"><img src="../product_pic/no' . $p_id . '.svg" style="width:100pt; display:block;" alt="" />' . $p_name[$p_id] . '</div>
